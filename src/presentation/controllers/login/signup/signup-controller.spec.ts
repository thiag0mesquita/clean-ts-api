import { SignUpController } from '../../login/signup/signup-controller'
import { AccountModel, AddAccount, HttpRequest, Validation, Authentication } from './signup-controller-protocols'
...
describe('Singup Controller', () => {
  ...
  test('Should return 200 if valid data is provided', async () => {
    const { sut } = makeSut()
    const httpReponse = await sut.handle(mockRequest())
    expect(httpReponse).toEqual(ok({ accessToken: 'any_token', name: 'any_name' }))
  })
